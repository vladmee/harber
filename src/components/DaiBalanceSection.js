...
class DaiBalanceSection extends Component {
  constructor(props, context) {
    ...
    this.state = {
      USD: -1,
      daiBalanceKey: context.drizzle.contracts.Cash.methods.balanceOf.cacheCall(
        address
      ),
      daiBalance: -1,
      patron: null
  ...
  async componentWillUpdate(nextProps, nextState) {
    if (
      this.state.daiBalanceKey in this.props.contracts["Cash"]["balanceOf"] &&
      this.state.daiBalanceKey in nextProps.contracts["Cash"]["balanceOf"]
    ) {
      if (
        this.getBalanceOf(this.props) !== this.getBalanceOf(nextProps) ||
        this.state.daiBalance === null
      ) {
